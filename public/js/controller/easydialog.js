...
Dialog.prototype = {
	...
	createDialogWrap : function( tmpl ){
		...
		var header = tmpl.header ? 
			...
			'',
			// 确定按钮
			proBtn = typeof tmpl.proFn === 'function' ? 
				'<button class="btn_product" id="easyDialogProBtn">' + ( typeof tmpl.proText === 'string' ? tmpl.proText : '待发布' ) + '</button>' :
				'', 
			// 确定按钮
			...
			noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
				...
				'',			
			// footer
			footer = yesBtn === '' && noBtn === '' && proBtn === '' ? '' :
				'<div class="easyDialog_footer">' + noBtn + yesBtn + proBtn + '</div>',
...
var extend = {
	open : function(){
		...
		var closeBtn = doc.getElementById( 'closeBtn' ),
			...
			dialogProBtn = doc.getElementById('easyDialogProBtn'),
			dialogYesBtn = doc.getElementById('easyDialogYesBtn'),
			dialogNoBtn = doc.getElementById('easyDialogNoBtn');		

		// 绑定确定按钮的回调函数
		if( dialogProBtn ){
			event.bind( dialogProBtn, 'click', function( event ){
				if( options.container.proFn.call(self, event) !== false ){
					self.close();
				}
			});
		}
	...
	close : function(){
		...
		if( elements.dialogProBtn ){
			event.unbind( elements.dialogProBtn, 'click' );
		}
